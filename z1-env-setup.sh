...
grep -q '/mnt/cs' /etc/fstab.tmp || echo '\/dev\/disk\/by-uuid\/9ec8096a-fbf5-4e38-9a44-80bad3173ab7 \/mnt\/cs auto nosuid,nodev,nofail,noatime,x-gvfs-show,discard,x-gvfs-name=cs 0 0' | sudo tee -a /etc/fstab.tmp
...
grep -q '/mnt/green' /etc/fstab.tmp || echo '\/dev\/disk\/by-uuid\/76FD-CAD8 \/mnt\/green auto nosuid,nodev,nofail,noatime,x-gvfs-show,x-gvfs-name=green,uid=1000,gid=1000 0 0' | sudo tee -a /etc/fstab.tmp
...
echo mdns;                sudo ufw allow in proto udp to 224.0.0.251 port 5353
